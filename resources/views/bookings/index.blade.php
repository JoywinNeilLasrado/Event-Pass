<x-app-layout>
            ...
            <div class="card overflow-hidden">
                ...
                @else
                    <div class="divide-y divide-gray-50 dark:divide-white/5 transition-colors">
                        @foreach($groupedBookings as $eventId => $group)
                            @php 
                                $event = $group->first()->event; 
                                $totalTickets = $group->count();
                                $totalPaid = $group->sum('amount_paid');
                                $bookingCreatedAt = $group->first()->created_at;
                                $firstTicketType = $group->first()->ticketType;
                            @endphp
                                ...
                                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                    <div>
                                        <div class="flex items-center gap-3 flex-wrap mb-1">
                                            ...
                                            @endif
                                            <span class="text-[10px] bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300 rounded-md px-2 py-0.5 font-bold uppercase tracking-widest shadow-sm">{{ $totalTickets }} Ticket(s)</span>
                                        </div>
                                        ...
                                        <p class="text-[11px] font-semibold text-gray-400 dark:text-gray-500 mt-3 uppercase tracking-wide transition-colors">
                                            Booked on {{ $bookingCreatedAt->format('M d, Y') }} 
                                            @if($totalPaid > 0)
                                                &bull; Paid ₹{{ number_format($totalPaid, 2) }} Total
                                            @elseif($firstTicketType && $firstTicketType->price > 0 && $totalPaid == 0)
                                    ...
                                    @if(!$event->deleted_at)
                                        <div class="flex-shrink-0 mt-4 sm:mt-0 flex items-center gap-3">
                                            ...
                                            <button type="button" x-data="" x-on:click.prevent="$dispatch('open-modal', 'cancel-booking-{{ $event->id }}')" class="btn-vercel-secondary text-xs px-4 py-2 hover:bg-red-50 dark:hover:bg-red-900/30 hover:text-red-600 dark:hover:text-red-400 hover:border-red-200 dark:hover:border-red-800 transition-colors focus:outline-none">
                                                Cancel
                                            </button>
                                        </div>
    ...
    @foreach($groupedBookings as $eventId => $group)
        ...
        <x-modal name="cancel-booking-{{ $event->id }}" :show="false" focusable>
            <form method="post" action="{{ route('bookings.destroy', $event->id) }}" class="p-6 bg-white dark:bg-gray-900 border border-gray-100 dark:border-white/10 rounded-xl">
                @csrf
                @method('delete')

                <h2 class="text-lg font-bold text-gray-900 dark:text-gray-100 mb-2">
                    Cancel {{ $event->title }} Tickets
                </h2>

                <p class="text-sm text-gray-600 dark:text-gray-400">
                    You currently hold <span class="font-bold text-black dark:text-white">{{ $totalTickets }} {{ $totalTickets == 1 ? 'ticket' : 'tickets' }}</span> for this event. 
                    How many would you like to cancel and refund?
                </p>

                <div class="mt-6 bg-gray-50 dark:bg-black/40 rounded-lg p-4 border border-gray-100 dark:border-white/5">
                    <label for="quantity-{{ $event->id }}" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Quantity to Cancel</label>
                    <select id="quantity-{{ $event->id }}" name="quantity" class="block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-black dark:focus:border-white focus:ring-black dark:focus:ring-white rounded-md shadow-sm">
                        @for($i = 1; $i <= $totalTickets; $i++)
                            <option value="{{ $i }}" {{ $i == $totalTickets ? 'selected' : '' }}>
                ...
                <div class="mt-6 flex justify-end gap-3">
                    <button type="button" x-on:click="$dispatch('close')" class="btn-vercel-secondary text-sm px-4 py-2">
                        Keep My Tickets
                    </button>
                    <button type="submit" class="bg-red-600 hover:bg-red-700 text-white font-bold rounded-lg text-sm px-6 py-2 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 border border-red-700 shadow-sm">
