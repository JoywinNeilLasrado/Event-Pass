...
<body
    class="antialiased bg-[#FAFAFA] dark:bg-black bg-mesh-light dark:bg-mesh-dark text-[#111827] dark:text-gray-100 flex flex-col min-h-screen transition-colors relative overflow-x-hidden">

    <header
        class="sticky top-0 z-50 bg-white/40 dark:bg-black/40 backdrop-blur-2xl border-b border-white/40 dark:border-white/10 transition-colors duration-300">
        <div class="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
            <div class="flex items-center gap-2">
                <span class="text-2xl">🎟️</span>
                <span
                    class="font-extrabold text-xl tracking-tighter text-gray-900 dark:text-white transition-colors">Passage.</span>
            </div>

            <nav class="flex items-center gap-6">
                <!-- Theme Toggle -->
                <button id="theme-toggle" type="button"
                    class="text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white transition-colors focus:outline-none hidden sm:block">
                    <svg id="theme-toggle-dark-icon" class="hidden w-5 h-5 flex-shrink-0" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z">
                        </path>
                    </svg>
                    <svg id="theme-toggle-light-icon" class="hidden w-5 h-5 flex-shrink-0" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="max-w-5xl mx-auto text-center">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6 relative z-20 mb-16">
                ...
                <a href="{{ route('register') }}"
                    class="btn-vercel text-lg px-10 py-4 w-full sm:w-auto shadow-xl shadow-purple-500/20 hover:shadow-purple-500/40 transition-all duration-300">Start
                    Building Events</a>
                <a href="{{ route('events.index') }}"
                    class="btn-vercel-secondary text-lg px-10 py-4 w-full sm:w-auto bg-white/50 dark:bg-white/5 backdrop-blur-md border-gray-200/50 dark:border-white/10 hover:bg-white/80 dark:hover:bg-white/15 transition-all duration-300 text-gray-900 dark:text-white">Explore
                    Events</a>
                @endauth
            </div>
            ...
            <div class="mt-24 sm:mt-32 w-full max-w-[100vw] overflow-hidden" x-data="{
                ...
                <div class="flex items-center justify-between mb-8 px-4 sm:px-0 max-w-7xl mx-auto">
                    <div class="text-left">
                        <h2 class="text-3xl sm:text-4xl font-black text-gray-900 dark:text-white tracking-tight">
                            Featured Events</h2>
                        <p class="text-gray-500 dark:text-gray-400 mt-2">Discover premium experiences curated just for
                            you.</p>
                    </div>
                    <div class="flex gap-2">
                        <button @click="prev()"
                            class="p-2 sm:p-3 rounded-full bg-white dark:bg-white/10 hover:bg-gray-50 dark:hover:bg-white/20 border border-gray-200 dark:border-white/10 shadow-sm transition-all focus:outline-none">
                ...
                <div class="relative w-full max-w-7xl mx-auto px-4 sm:px-0">
                    <div
                        ...
                        <div class="flex w-full h-full transition-transform duration-700 ease-in-out"
                            ...
                            <div class="w-full h-full flex-shrink-0 relative group cursor-pointer"
                                ...
                                @else
                                <div
                                    class="absolute inset-0 w-full h-full bg-gradient-to-tr from-indigo-500 to-purple-600">
                                </div>
                                @endif
                                ...
                                <!-- Content -->
                                <div
                                    class="absolute inset-0 p-6 sm:p-12 flex flex-col justify-end text-left pb-12 sm:pb-16 text-white">
                                    <div class="flex flex-wrap gap-2 mb-4">
                                        <span
                                            class="px-3 py-1 bg-white/20 backdrop-blur-md rounded-full text-xs font-bold uppercase tracking-wider">{{
                                            $event->category ? $event->category->name : 'Event' }}</span>
                                        <span
                                            class="px-3 py-1 bg-emerald-500/80 backdrop-blur-md rounded-full text-xs font-bold uppercase tracking-wider">Featured</span>
                                        <span
                                            class="px-3 py-1 bg-black/40 backdrop-blur-md rounded-full text-xs font-bold uppercase tracking-wider"><svg
                                                class="w-3 h-3 inline mr-1 -mt-0.5" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z">
                                                </path>
                                            </svg>{{ $event->date->format('M j, Y') }}</span>
                                    </div>
                                    <h3
                                        class="text-3xl sm:text-5xl md:text-6xl font-black mb-2 sm:mb-4 tracking-tight drop-shadow-md">
                                        {{ $event->title }}</h3>
                                    <p class="text-sm sm:text-lg text-white/80 line-clamp-2 max-w-3xl drop-shadow mb-6">
                                        {{ $event->description }}</p>

                                    <span
                                        class="inline-flex items-center gap-2 text-sm sm:text-base font-bold group-hover:text-purple-300 transition-colors">
                                        Get Tickets
                    ...
                    <!-- Slide Indicators -->
                    <div
                        class="absolute bottom-6 sm:bottom-8 left-1/2 -translate-x-1/2 flex items-center justify-center gap-2 sm:gap-3 z-20">
                        <template x-for="(slide, index) in slides" :key="index">
                            <button @click="activeSlide = index"
                                class="w-12 sm:w-16 h-1 sm:h-1.5 rounded-full transition-all duration-300"
                                :class="activeSlide === index ? 'bg-white' : 'bg-white/30 hover:bg-white/50'"></button>
                        </template>
                    </div>
                </div>
            </div>
    ...
    <footer
        class="border-t border-gray-200 dark:border-white/10 bg-white/50 dark:bg-[#0A0A0A]/50 backdrop-blur-md pt-16 pb-8 mt-auto transition-colors relative z-20">
