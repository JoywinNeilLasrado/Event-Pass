<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('events.show', $event) }}" class="text-gray-400 hover:text-black transition flex items-center">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
            </a>
            <h2 class="font-extrabold text-2xl text-gray-900 tracking-tight">Edit Event</h2>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="card bg-white p-8 sm:p-12 shadow-sm border border-gray-100">
                ...
                <div class="mb-8">
                    <h3 class="text-2xl font-bold text-gray-900 tracking-tight">Event Settings</h3>
                    <p class="text-sm text-gray-500 mt-1">Update the fundamental details of your occurrence.</p>
                </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="5" required
                                class="w-full rounded-lg border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-900 focus:border-black focus:ring-black focus:bg-white transition-colors placeholder-gray-400" placeholder="Provide a detailed overview of what to expect...">{{ old('description', $event->description) }}</textarea>
                        </div>
                    </div>
                        ...
                        <div class="flex flex-col sm:flex-row gap-6 items-start">
                            @if($event->poster_image)
                                <div class="relative group flex-shrink-0">
                                    <img src="{{ Storage::url($event->poster_image) }}" alt="Current poster" class="h-32 w-48 rounded-lg object-cover border border-gray-200 shadow-sm">
                                </div>
                            @endif
